...
def make_dict(json_file):
    ...
    for param in summary["summary"]["before_filtering"].keys():
        final_summary["before_filtering_" + param] = summary["summary"][
            "before_filtering"
        ][param]
    for param in summary["summary"]["after_filtering"].keys():
        final_summary["after_filtering_" + param] = summary["summary"][
            "after_filtering"
        ][param]
...
def writeTSV(final_summary):
    ...
    with open(
        final_summary["isolate"] + "_inter_report.tsv", "w", newline=""
    ) as output_file:
        dict_writer = csv.DictWriter(
            output_file, fieldnames=agg_summary[0].keys(), delimiter="\t"
        )
