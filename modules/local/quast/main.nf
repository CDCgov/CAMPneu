process QUAST {
    ...
    tuple val(meta), path("${prefix}_transposed_report.tsv") , emit: transposed
    tuple val(meta), path("${prefix}_reads.tsv")             , optional: true , emit: reads
    ...
    def in_reads  = reads           ? "-1 ${reads[0]} -2 ${reads[1]}" : ''
    """
    quast.py \\
        --output-dir $prefix \\
        $in_reads \\
